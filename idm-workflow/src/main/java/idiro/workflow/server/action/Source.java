...
public class Source extends DataflowAction {
			...
			public String check(DFEPage page) throws RemoteException {
				...
				try {
					...
					if (error == null) {
						...
						Tree<String> datasetTree = getInteraction(key_dataset)
								.getTree();
						logger.info("Tree : "+((TreeNonUnique<String>) datasetTree).toString());
						if (!datasetTree.getFirstChild("browse")
								.getFirstChild("output")
								.getFirstChild("property").isEmpty()) {
	...
	public void updateDataSet(Tree<String> treeDataset) throws RemoteException {
		...
		if (treeDataset.getSubTreeList().isEmpty()) {
			...
			treeDataset.getFirstChild("browse").getFirstChild("output").add("property");
