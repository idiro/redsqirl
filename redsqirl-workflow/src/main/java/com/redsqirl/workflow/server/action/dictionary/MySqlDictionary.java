...
public class MySqlDictionary extends JdbcDictionary{
	...
	protected void loadDefaultFunctions() {
		...
		functionsMap.put(castMethods, new String[][] { 
			...
			+ "@short:returns the date value of the object following given in the format."+dateFormats+
			"@example: STR_TO_DATE('20160201','%y%M%d')"
			},
			new String[] { "TIMESTAMP()", "STRING", "DATETIME",
			"@function:TO_DATE"
			+ "@short:returns the date value of the object.@example: TIMESTAMP('2016-02-01')"
			}
		});
		...
		String[][] extraMathFunctions = new String[][] {
			...
			"@function:TRUNC"
			+ "@short:returns a the number truncated."
			+ "@param:X the number to truncate"
			+ "@param:D the precision to keep."
		...
		String[][] mySqlStringMethods = new String[][] {
			...
			+ "@short:Extracts the year from String timestamps."
			+ "@param:STRING The string that contains a date."
			+ "@description:Returns the year part of a timestamp string"
			+ "@example: YEAR(\"1970-01-01\") returns \"1970\"" 
			},
			new String[] { "MONTH()", "STRING",	"INT",
			"@function:MONTH(STRING)"
			+ "@short:Extracts the MONTH from String timestamps "
			+ "@param:STRING the string that contains a date"
			+ "@description:Returns the MONTH part of a timestamp string"
			+ "@example: MONTH(\"1970-11-01 \") returns \"11\"" 
			},
			new String[] { "DAY()", "STRING", "INT",
			"@function:DAY(STRING)"
			+ "@short:Extracts the DAY from String timestamps "
			+ "@param:STRING the string that contains a date"
		...
		String[][] mySqlDateMethods = new String[][] {
			new String[] { "FROM_UNIXTIME()", "LONG", "TIMESTAMP",
			...
			new String[] { "FROM_UNIXTIME()", "LONG,STRING", "TIMESTAMP",
			...
			+"@description: Converts timestamp to Unix timestamp (in seconds), using the default timezone and the default locale, return 0 if fail"
			...
			+"@param:STR_DATE The string date to convert."
			+ "@param:FORMAT The date format."+dateFormats
			+"@description: Convert time string with given pattern to Unix time stamp (in seconds), return 0 if fail"
			+"@example:unix_timestamp('2009-03-20', 'yyyy-MM-dd') = 1237532400"
			},
			new String[] { "YEAR()", "TIMESTAMP", "INT",
			"@function:YEAR(TIMESTAMP)"
			+"@short: Returns the year part of a date or a timestamp"
			+"@param:TIMESTAMP A timestamp"
			+"@description: Returns the year part of a date or a timestamp"
			+"@example:year('1970-01-01 00:00:00') = 1970, year('1970-01-01') = 1970"
			},
			new String[] { "MONTH()", "TIMESTAMP", "INT",
			"@function:MONTH(TIMESTAMP)"
			+"@short:Returns the month part of a date or a timestamp."
			+"@param:TIMESTAMP A timestamp"
			+"@description: Returns the month part of a date or a timestamp"
			+"@example:month('1970-11-01 00:00:00') = 11"
			+"@example:month('1970-11-01') = 11"
			},
			new String[] { "DAY()", "TIMESTAMP", "INT",
			"@function:DAY(TIMESTAMP)"
			+"@short: Return the day part of a date or a timestamp."
			+"@param:TIMESTAMP A timestamp"
			+"@description:Return the day part of a date or a timestamp"
			+"@example:day('1970-11-01 00:00:00') = 1"
			+"@example:day('1970-11-01') = 1"
			},
			new String[] { "HOUR()", "TIMESTAMP", "INT",
			"@function:HOUR(TIMESTAMP)"
			+"@short:Returns the hour of the timestamp."
			+"@param:TIMESTAMP A timestamp"
			+"@description:Returns the hour of the timestamp"
			+"@example:hour('2009-07-30 12:58:59') = 12"
			+"@example:hour('12:58:59') = 12"
			},
			...
			+"@short: Return the week number of a timestamp string"
			+"@param:TIMESTAMP A timestamp"
			+"@example:weekofyear('1970-11-01 00:00:00') = 44"
			+"@example:weekofyear('1970-11-01') = 44"
			},
