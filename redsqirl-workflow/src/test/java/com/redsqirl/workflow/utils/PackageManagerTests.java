...
public class PackageManagerTests{
	...
	public void basic(){
		...
		try{
			...
			assertTrue("Fail to add "+packName,	pkm.addPackage(user, new String[]{pack.getAbsolutePath()}) == null);

			assertFalse("Success to add "+packName,	pkm.addPackage(user, new String[]{pack.getAbsolutePath()}) == null);	

			assertFalse("Success to add "+packName3, pkm.addPackage(user, new String[]{pack3.getAbsolutePath()}) == null);

			assertTrue("Success to add "+packName2,	pkm.addPackage(user, new String[]{pack2.getAbsolutePath()}) == null);

			logger.debug("Remove packages....");
			assertTrue("Fail to remove "+packName, pkm.removePackage(user, new String[]{packName}) == null);

			assertFalse("Fail to remove "+packName,	pkm.removePackage(user, new String[]{packName}) == null);

			assertTrue("Fail to remove "+packName2,	pkm.removePackage(user, new String[]{packName2}) == null);
