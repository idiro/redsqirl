...
/**
 * Output tab object associated with a CanvasModal object.
 * The output tab object control and configure the data 
 * associated with a DataFlowElement
 *  
 * @author etienne
 *
 */
public class CanvasModalOutputTab implements Serializable{
	...
	/**
	 * Generate the list of outputForm and check if the output form
	 * is ready to be displayed.
	 * 
	 * @param setShowOutputForm Don't change the status of the output form display.
	 * @throws RemoteException
	 */
	public void mountOutputForm(boolean setShowOutputForm) throws RemoteException {

		logger.info("mountOutputForm");
		if (dfe != null && dfe.getOozieAction() != null
				&& dfe.getDFEOutput() != null
				&& !dfe.getDFEOutput().isEmpty()) {

			outputFormList = new LinkedList<OutputForm>();

			for (Entry<String, DFEOutput> e : dfe.getDFEOutput()
					.entrySet()) {
				OutputForm of = new OutputForm(e.getValue(), dfe
	...
	/**
	 * Display the first rows of data available from the given output.
	 * An outputName parameter is necesseray for running this function.
	 * @throws RemoteException
	 */
	public void displayOutput() throws RemoteException {
		String outputN = FacesContext.getCurrentInstance()
	...
	/**
	 * Update the rows of data from the current output.
	 *  The rows of data are given in the gridTitle and
	 * outputGrid fields.
	 * @throws RemoteException
	 */
				...
				if (dfeOut != null && 
					...
					try{
						...
						for (String outputFeature : outputFeatureList) {
							...
							gridTitle.add(outputFeature + " " + featureType.toString());
						}
	...
	/**
	 * Remove the data associated with a given output name.
	 * The output name is given through the context "nameOutputClean"
	 * @throws RemoteException
	 */
	...
	/**
	 * @param outputFormList the outputFormList to set
	 */
	...
	/**
	 * @param showOutputForm the showOutputForm to set
	 */
	...
	/**
	 * @param nameOutput the nameOutput to set
	 */
