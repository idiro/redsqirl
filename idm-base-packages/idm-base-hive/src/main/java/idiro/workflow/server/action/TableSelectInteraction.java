...
public class TableSelectInteraction extends UserInteraction {
	...
	public TableSelectInteraction(String name, String legend, int column,
			int placeInColumn, HiveSelect hs) throws RemoteException {
	...
	public String check() throws RemoteException {
		...
		try {
			lRow = getTree().getFirstChild("table").getFirstChild("generator")
		...
		while (rows.hasNext() && msg == null) {
			...
			logger.info("row: "+((TreeNonUnique<String>) row).toString());
			if (row.getChildren(table_type_title).size() != 1
				...
				try {
					String val = row.getFirstChild(table_op_title)
							.getFirstChild().getHead();
					String type = row.getFirstChild(table_type_title).getFirstChild()
							.getHead();
					logger.info("type : "+ type + " value :"+val);
	...
	public void update(DFEOutput in) throws RemoteException {
		...
		if (tree.isEmpty() || tree.getSubTreeList().isEmpty()) {
			logger.info("generating root tree");
			tree.add(getRootTable());
		} else {
			logger.info("removing generator and columns from tree");
			// Remove generator
		...
		Iterator<String> gbFeats = hs.getInFeatures().getFeaturesNames()
				.iterator();
		if (gbFeats.hasNext()) {
