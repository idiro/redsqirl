...
public class PigGroupRankTests {
	...
	public static void updatePig(Workflow w, PigGroupRank pig,
		...
		String rnk = vals.remove(vals.size());
	...
	public void basic() {
		...
		try {
			...
			/*
			 * logger.info("run..."); OozieClient wc =
			 * OozieManager.getInstance().getOc();
			 * logger.info("Got Oozie Client"); error = w.run();
			 * assertTrue("Job submition failed: "+error, error == null); String
			 * jobId = w.getOozieJobId(); if(jobId == null){
			 * assertTrue("jobId cannot be null", false); } logger.info(jobId);
			 * 
			 * // wait until the workflow job finishes printing the status every
			 * 10 seconds while( wc.getJobInfo(jobId).getStatus() ==
			 * org.apache.oozie.client.WorkflowJob.Status.RUNNING) {
			 * System.out.println("Workflow job running ...");
			 * logger.info("Workflow job running ..."); Thread.sleep(10 * 1000);
			 * } logger.info("Workflow job completed ...");
			 * logger.info(wc.getJobInfo(jobId)); error =
			 * wc.getJobInfo(jobId).toString(); assertTrue(error,
			 * error.contains("SUCCEEDED"));
			 */
		} catch (Exception e) {
