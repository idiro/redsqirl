...
public class MrqlTestUtils {

	private static Logger logger = Logger.getLogger(MrqlTestUtils.class);
	...
	public static DataFlowElement createSrc_ID_VALUE_RAW(Workflow w, HDFSInterface hInt,
		...
		MrqlTestUtils.create3INT_file(new Path(new_path1));
		
		String idSource = w.addElement((new MrqlTextSource()).getName());
		MrqlTextSource src = (MrqlTextSource) w.getElement(idSource);

		src.update(src.getInteraction(MrqlTextSource.key_dataset));
		Tree<String> dataSetTree = src.getInteraction(MrqlTextSource.key_dataset)
				.getTree();
...
//		src.getDFEOutput().get("").addProperty(MapRedTextType.key_header, "ID INT, VALUE INT, RAW INT");
		...
		assertTrue("number of fields in source should be 3 instead of " + 
				src.getDFEOutput().get(MrqlTextSource.out_name).getFields().getSize(),
				src.getDFEOutput().get(MrqlTextSource.out_name).getFields().getSize() == 3);
		...
		assertTrue("field list " + 
				src.getDFEOutput().get(MrqlTextSource.out_name).getFields().getFieldNames(),
				src.getDFEOutput().get(MrqlTextSource.out_name).getFields().getFieldNames().containsAll(fields));
	...
	public static DataFlowElement createSourceEmpty_ID_VALUE(Workflow w,String path) throws Exception{
		...
		src.update(src.getInteraction(MrqlTextSource.key_dataset));
		Tree<String> dataSetTree = src.getInteraction(MrqlTextSource.key_dataset).getTree();
		...
		src.getDFEOutput().get(MrqlTextSource.out_name).setFields(fl);
		
		assertTrue("number of fields in source should be 2 instead of " + 
				src.getDFEOutput().get(MrqlTextSource.out_name).getFields().getSize(),
				src.getDFEOutput().get(MrqlTextSource.out_name).getFields().getSize() == 2);
	...
	public static DataFlowElement createSrc_ID_VALUE(
		...
		assertTrue("field list " + 
				src.getDFEOutput().get(MrqlTextSource.out_name).getFields().getFieldNames(),
				src.getDFEOutput().get(MrqlTextSource.out_name).getFields().getFieldNames().contains("ID"));
		assertTrue("field list " + 
				src.getDFEOutput().get(MrqlTextSource.out_name).getFields().getFieldNames(),
				src.getDFEOutput().get(MrqlTextSource.out_name).getFields().getFieldNames().contains("VALUE"));
	...
	public static DataFlowElement createSrc_ID_2VALUE(
		...
		assertTrue("field list " + 
				src.getDFEOutput().get(MrqlTextSource.out_name).getFields().getFieldNames(),
				src.getDFEOutput().get(MrqlTextSource.out_name).getFields().getFieldNames().contains("VALUE2"));
